...
bsafe_UI <- function(id, header = NULL) { # nolint
  ...
  shiny::tagList(
    shinyjs::useShinyjs(),
    shiny::includeCSS(system.file("www/bsafe.css", mustWork = TRUE, package = "dv.modules.bsafe")),
    shiny::sidebarLayout(
        ...
        shiny::validate(
          ...
          shiny::need(
            checkmate::test_character(data[["STUDYID"]]) ||
              checkmate::test_factor(data[["STUDYID"]]),
          ...
          shiny::need(
            checkmate::test_character(data[["SAF_TOPIC"]]) ||
              checkmate::test_factor(data[["SAF_TOPIC"]]),
            "SAF_TOPIC needs to be a character"
          ),
          shiny::need(
            checkmate::test_character(data[["ARM"]]) ||
              checkmate::test_factor(data[["ARM"]]),
...
mod_bsafe <- function(module_id, dataset_name) {
  mod <- list(
    ui = bsafe_UI,
