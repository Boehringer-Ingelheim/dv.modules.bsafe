...
mod_robust_map_server <- function(
  ...
  mod <- function(input, output, server) {
    ...
    robust_map_mcmc <- shinymeta::metaReactive2(
      {
        ...
        # TODO: dv.bsafe::robust_map cannot handle values below than 1, because a log is calculated in bsafe and then RBest
    ...
    robust_plot <- shinymeta::metaReactive(
      {
        dv.bsafe::robust_map_prior_plot( # nolint: object_usage_linter
          rob_comp = ..(rob_comp()),
    ...
    robust_summary <- shinymeta::metaReactive2(
      {
        ...
        shinymeta::metaExpr(
          dv.bsafe::summary_stats_robust_map_prior_display( # nolint: object_usage_linter
            map_object = ..(map_mcmc()),
            select_analysis = ..(analysis_type()),
            param_approx = ..(param_approx()),
    ...
    robust_formula <- shinymeta::metaReactive(
      {
        shinymeta::metaExpr({
          dv.bsafe::robust_map_prior_mix_dens_display(
            robust_map_object = ..(robust_map_mcmc()),
            select_analysis = ..(analysis_type())
          )
        })
...
mock_robust_map_mod <- function(analysis_type = BSAFE_CHOICES$SEL_ANALYSIS[1]) {
  ...
  server <- function(input, output, session) {
    metareact_in <- purrr::imap(dv.modules.bsafe::test_rm_in, function(v, n) {
      shinymeta::metaReactive(
        {
          rlang::quo(dv.modules.bsafe::test_rm_in[[!!n]])
        },
        quoted = TRUE,
        varname = n
      )
