...
mod_decision_making_server <- function(
  ...
  mod <- function(input, output, session) {
    ...
    stat_inf_plot <- shinymeta::metaReactive2({
      ...
      r_ae_prop <- ae_prop()
      shinymeta::metaExpr({
        dv.bsafe::decision_making_density_plot(
...
mock_decision_making_mod <- function(analysis_type = BSAFE_CHOICES$SEL_ANALYSIS[1]) {
  ...
  server <- function(input, output, session) {
    trial_in_metareact <- purrr::imap(dv.modules.bsafe::test_dm_in, function(v, n) {
      shinymeta::metaReactive(
        {
          rlang::quo(dv.modules.bsafe::test_dm_in[[!!n]])
        },
        quoted = TRUE,
        varname = n
      )
