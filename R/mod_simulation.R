...
mod_simulation_server <- function(id, data, tmpfolder) {
  mod <- function(input, output, session) {
    ...
    shiny::observeEvent(input[[BSAFE_ID$BUT_COMP_SUBMIT]], {
      ...
      withCallingHandlers(
        {
          ae_summary_data <<- dv.bsafe::ae_summary_table(
            data(),
            cb_list_ctrl,
            cb_list_trt,
      ...
      rmarkdown::render(
        ...
        params = list(
          ...
          bsafe_version = utils::packageVersion("dv.modules.bsafe"),
...
mock_simulation_mod <- function() {
  ...
  server <- function(input, output, session) {
    x <- mod_simulation_server(
      id = "mock",
      data = shiny::reactive(as.data.frame(dv.modules.bsafe::bsafe_data))
    )
